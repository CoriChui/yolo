---
name: yolo-init
description: Use when setting up YOLO in a project for the first time, or repairing the setup. Scaffolds workspace/config.yaml and the features/, intake/, decisions/, and debug/ dirs, and installs the CLAUDE.md routing block. Triggers on "set up YOLO here", "init yolo", "yolo init".
---
...
## Procedure
1. Create `workspace/`, `workspace/features/`, `workspace/intake/`, `workspace/decisions/`, `workspace/debug/` if missing (each tracked; add a `.gitkeep` where empty).
